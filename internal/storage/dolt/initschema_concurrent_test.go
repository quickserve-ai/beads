...
func TestConcurrentInitSchema(t *testing.T) {
	...
	for i := 0; i < numConcurrent; i++ {
		...
		go func(n int) {
			...
			if _, err := initSchemaOnDBWithRetry(ctx, db, 0); err != nil {
...
func TestInitSchemaBlocksOnMigrationLock(t *testing.T) {
	...
	go func() {
		_, err := initSchemaOnDBWithRetry(ctx, initDB2, 0)
		errCh <- err
	}()
...
func TestInitSchemaCanceledLockWaitDoesNotBlockFutureInit(t *testing.T) {
	...
	errCh := make(chan error, 1)
	go func() {
		_, err := initSchemaOnDBWithRetry(callerCtx, blockedDB, 0)
		errCh <- err
	}()
	...
	defer verifyCancel()
	if _, err := initSchemaOnDBWithRetry(verifyCtx, secondDB, 0); err != nil {
